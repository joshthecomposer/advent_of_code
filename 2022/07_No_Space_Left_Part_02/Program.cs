...
long diskGoal = 30000000;

long diskUsed = rootDir.TotalSize;

long unusedSize = totalDisk - diskUsed;
long missingSize = diskGoal - unusedSize;

Directory best = rootDir;

TraverseDirs(best);
Console.WriteLine(best.TotalSize);

void TraverseDirs(Directory source)
{
    if (missingSize <= source.TotalSize && best.TotalSize > source.TotalSize)
    {
        best = source;
    }
